...
/**
 * PATCH /api/services_proxy_route/[serviceId]
...
export const PATCH = withApiAuthRequired(async function handlePatch(req: NextRequest, context: HandlerContext) {
  try {
    ...
    console.error(`Error in PATCH /api/services_proxy_route/${context.params?.serviceId}:`, error);
    return NextResponse.json({ error: error.message || 'Internal Server Error' }, { status: error.status || 500 });
  }
});

/**
 * DELETE /api/services_proxy_route/[serviceId]
...
export const DELETE = withApiAuthRequired(async function handleDelete(req: NextRequest, context: HandlerContext) {
  try {
    ...
    console.error(`Error in DELETE /api/services_proxy_route/${context.params?.serviceId}:`, error);
    return NextResponse.json({ error: error.message || 'Internal Server Error' }, { status: error.status || 500 });
  }
});
